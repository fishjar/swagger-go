...
func RoleFindAndCountAll(c *gin.Context) {
	...
	var rows []model.Role
	var count uint
	if err := db.DB.Model(&rows).Where(where).Count(&count).Limit(pageSize).Offset(offset).Order(order).Preload("Users").Preload("Menus").Find(&rows).Error; err != nil {
...
func RoleFindByPk(c *gin.Context) {
	...
	// 查询
	var data model.Role
	if err := db.DB.Where("id = ?", id).Preload("Users").First(&data).Error; err != nil {
